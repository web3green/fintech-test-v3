...
if (process.argv.length < 3) {
  console.error('Использование: node update-logo-only.js путь/к/логотипу.png');
  process.exit(1);
}
...
try {
  ...
  fs.copyFileSync(logoPath, path.join(imagesDir, 'logo.png'));
  console.log(`Логотип скопирован в: ${path.join(imagesDir, 'logo.png')}`);
  ...
  if (fs.existsSync(emergencyHtmlPath)) {
    ...
    // Обновляем логотип в аварийной странице
  ...
  if (fs.existsSync(distDir)) {
    ...
    fs.copyFileSync(logoPath, path.join(distImagesDir, 'logo.png'));
    console.log(`Логотип скопирован в: ${path.join(distImagesDir, 'logo.png')}`);
  }
  
  console.log('\nЛоготип успешно обновлен во всех местах использования!');
  console.log('Формат изменен с SVG на PNG.');
